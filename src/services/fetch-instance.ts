...
import { misskey } from '@/libs/misskey';

export const fetchInstanceV1 = async (): Promise<any> => {
  const meta = await misskey.request('meta', { detail: true });
  const stats = await misskey.request('stats');
  const domain = new URL(meta.uri).host;
  const donkeyUrl = new URL(config.host ?? '');
...
export const fetchInstanceV2 = async (): Promise<any> => {
  const meta = await misskey.request('meta', { detail: true });
  const domain = new URL(meta.uri).host;
  const donkeyUrl = new URL(config.host ?? '');
